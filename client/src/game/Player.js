...
export class Player {
  ...
  update(dt, input, world, cameraController) {
    ...
    if (moveInput.x !== 0 || moveInput.z !== 0) {
      ...
      // Movement checks: not in water, and not colliding with shop or trees
      const canMove = (x, z) => {
        return !world.isInWater(x, z) && 
               !world.collidesWithShop(x, z, PLAYER.BODY_RADIUS) &&
               !world.collidesWithTrees(x, z, PLAYER.BODY_RADIUS);
      };

      const isSprinting = input.isDown('ShiftLeft') || input.isDown('ShiftRight');
      const speedMultiplier = isSprinting ? PLAYER.SPRINT_MULTIPLIER : 1.0;
    ...
    if (this.isGrounded && this.position.y > terrainY + 0.05) {
      this.isGrounded = false;
    ...
    // Update rod animation and line positions if fishing or reeling
    if (this._rodGroup) {
